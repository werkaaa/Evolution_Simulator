    ...
    public void generateMap() {
        ...
        for(int y = 0; y <this.height; y++) {
            for(int x = 0; x < width; x++) {
                ...
                Object mapElement;
                Object selectedOne = this.map.getMapData().getSelectedAnimal();
                JLabel label = grids.get(position);
                if(selectedOne!=null && ((Animal)selectedOne).getPosition().equals(position)){
                    mapElement = this.map.getMapData().getSelectedAnimal();
                ...
                String energyLevel = mapElement instanceof Animal ? (((Animal) mapElement).alive ? String.valueOf(min(4, ((Animal) mapElement).getEnergy()/(maxEnergy /5)))+"_" : "dead_"): "";
                String cellName = energyLevel+objectType+background;
                Icon element = pictures.get(cellName);
                label.setIcon(element);
//                if(mapElement instanceof Animal && ((Animal)mapElement).isChosen()) label.setBorder(BorderFactory.createLineBorder(Color.red));
